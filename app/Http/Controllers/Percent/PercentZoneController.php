...
class PercentZoneController extends Controller
{

     

    public function getData($paramprovince, $paramdistrict, $paramfacility)
    {
        ...
        $paramchildren = Percent::select('uuid', 'age',
                                  ...
                                  ->where('district', '=', $paramdistrict)
                                  ->where('health_facility', '=', $paramfacility)
        ...
        if (is_null($paramchildren)) {
         ...
         foreach ($paramchildren as $child) {
            ...
            $zone = $child->zone; 
            $paramzone = $child->dist_faci_zone; 

            if($current_zone!=$paramzone) {
                $current_zone=$paramzone;
                array_push($arry_zone, $current_zone); 
                 $allChildren = 0;
                 $allChildren_prov = 0; 
                 $allChildren_023 = 0;
                 $allChildren_prov023 = 0;
            ...
            if($child->province == $paramprovince and $child->dist_faci_zone == $paramzone)
            {  
            ...
            if($age >= 12 and $age < 24)
            {
              ...
              ${$current_zone} = array( 
                'zone'=>$zone,'facility'=>$paramfacility,'province'=>$paramprovince,'district'=>$paramdistrict,'fully'=>$percent_fully,
